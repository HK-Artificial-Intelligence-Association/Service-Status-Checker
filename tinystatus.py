...
async def monitor_services():
    ...
    while True:
        ...
        try:
            ...
            with open(HISTORY_TEMPLATE_FILE, 'r',encoding='utf-8') as f:
            ...
            with open(os.path.join(HTML_OUTPUT_DIRECTORY, 'index.html'), 'w',encoding='utf-8') as f:
                f.write(html)

            history_html = history_template.render(history=load_history(), last_updated=datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
            with open(os.path.join(HTML_OUTPUT_DIRECTORY, 'history.html'), 'w',encoding='utf-8') as f:
